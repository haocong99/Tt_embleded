...
void control_air_lg(void)
{
		...
		for(count = 0; count < NUMBER_REPEAT_SEND_CMD; count ++)
		{
				...
				start_condition_lg();
				/* send data 7 bytes dau */
				for(len = 0; len < LG_ON_LEN; len ++)
					sendbyte_data_lg(air_cmd.cmd_content[len]);
				stop_condition_lg();
...
void control_air_tcl()
{
		...
		for(count = 0; count < NUMBER_REPEAT_SEND_CMD; count ++)
		{
			...
			start_condition_tcl();
			/* send data 8 byte dau */
			for(len = 0; len < TCL_CMD_LEN; len ++)
				sendbyte_data_tcl(air_cmd.cmd_content[len]);
			stop_condition_tcl();
			HAL_IWDG_Refresh(&hwwdg);
			WATCHDOG_REFRESH();
		}
...
//		delay_us(71000);	
//}
//void restart_condition_nagakawa_tq(void)
//{
//		bit_0_nagakawa_tq();
//		AIR_LO();
//		delay_us(6090);	
//}
//void control_air_nagakawa_tq()
//{
//		uint8_t count  = 0;
//		uint8_t len  = 0;
//		/* Gui cau lenh */
//		for(count = 0; count < /*NUMBER_REPEAT_SEND_CMD*/1; count ++)
//		{
//			/* start condition */
//			start_condition_nagakawa_tq();
//			/* send data 8 byte dau */
//			for(len = 0; len < 6; len ++)
//				sendbyte_data_nagakawa_tq(air_cmd.cmd_content[len]);
//			restart_condition_nagakawa_tq();
//			HAL_IWDG_Refresh(&hwwdg);
//			WATCHDOG_REFRESH();			
//			for(len = 6; len < 14; len ++)
//				sendbyte_data_nagakawa_tq(air_cmd.cmd_content[len]);	
//			restart_condition_nagakawa_tq();
//			HAL_IWDG_Refresh(&hwwdg);
//			WATCHDOG_REFRESH();			
//			for(len = 14; len < NAGAKAWA_TQ_LEN; len ++)
//				sendbyte_data_nagakawa_tq(air_cmd.cmd_content[len]);				
//			stop_condition_nagakawa_tq();
//			HAL_IWDG_Refresh(&hwwdg);
//			WATCHDOG_REFRESH();		
//		}
//}