...
uint8_t CMD_READ_ID_FLASH								 =  0x9F; /* READY_STATUS id cua flash */
uint8_t CMD_READ_UID_FLASH								 =  0x4B; /* READY_STATUS uid cua flash */
/**
...
int8_t S25FL1_read_id_flash(flash_t *flash)
{
	uint8_t id_flash[4];
	id_flash[3] = 0;
	uint8_t uid_flash[20];
	memset(&uid_flash[0],0,20);
	/* Check status of flash chip */  
	if ( S25FL1_IsReady() == BUSY_STATUS)
	{
		return BUSY_STATUS;
	...
	delay_time_flash();
	/* Send command read SR1 */
	Send_Data(&CMD_READ_ID_FLASH,1, 0xff); 
	/* Read data */
	Read_Data(&id_flash[0], 3, 0xff);
	FLASH_SPI_CS_OFF();
	/* Gan uid */
	flash->manuafacturer = id_flash[0];
	flash->device_type = id_flash[1];	
	flash->capacity = id_flash[2];	
	/* Doc uid flash */
	HAL_Delay(100);
	/* Check status of flash chip */  
	if ( S25FL1_IsReady() == BUSY_STATUS)
	...
	delay_time_flash();
	/* Send command read SR1 */
	Send_Data(&CMD_READ_UID_FLASH,1, 0xff); 
	/* Read data */
	Read_Data(&uid_flash[0], 12, 0x1ff);
	FLASH_SPI_CS_OFF();	
	/* Lay UID flash */
	memcpy(&flash->uid_flash[0],&uid_flash[4],8);
	/* So sanh gia tri */
	return READY_STATUS;	
}
